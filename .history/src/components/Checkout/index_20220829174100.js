...
export default function Checkout() {
  ...
  const getDiscountFromCodeVoucher = () => {
    const indexCodeVoucher = dataVoucher.map(item => item.code).indexOf(codeVoucher)
    if (indexCodeVoucher !== -1) {
      return dataVoucher[indexCodeVoucher].discount
    }
  ...
  return (
    ...
    >
      ...
      <div className="p-8 basis-1/3 bg-slate-200">
        ...
        <Input placeholder="NHẬP MÃ GIẢM GIÁ" className="my-4" onChange={handleSetCodeVoucher} />
        ...
        <div className="pt-2 mb-4 border-t border-black">
          ...
          <p>
            Giảm giá:{' '}
            <span className="ml-1 font-semibold">
              {formatMoney(dataVoucher.includes}đ
            </span>
          </p>
        </div>
